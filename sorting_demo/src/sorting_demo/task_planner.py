...
class TaskPlanner:
    ...
    def find_next_block_pose(self):
        ...
        #rospy.logwarn("FIND NEXT BLOCK TASK")
        ...
        blocks = self.sawyer_robot.environment_estimation.get_blocks()
        ...
        if blocks is not None and len(blocks) > 0:
            self.target_block_pose = blocks[0].final_pose  # access first item , pose field
            ...
            #rospy.logwarn(   "blocks position:" + str(self.sawyer_robot.environmentEstimation.get_blocks()) + "original\n" + str(original_pose_block))

            # return self.target_block
        else:
            rospy.logwarn("OUPS!!")

        trays = self.sawyer_robot.environment_estimation.get_trays()

        if trays is not None and len(trays) > 0:
            self.target_tray_pose = trays[0].final_pose  # access first item , pose field
            self.target_tray_pose.orientation = overhead_orientation

            self.target_tray_pose.position.x += overhead_translation[0]
            self.target_tray_pose.position.y += overhead_translation[1]
            self.target_tray_pose.position.z += overhead_translation[2]

        rospy.logwarn("TRAYS: " + str(trays))

        rospy.logwarn("TARGET TRAY POSE: " + str(self.target_tray_pose))
    ...
    def async_main_task(self):
        ...
        while True:
            ...
            yield self.create_pick_task(self.target_block_pose, approach_speed=1.0)
            yield self.create_place_task(self.target_tray_pose)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            self.sawyer_robot.environment_estimation.update()
            rospy.sleep(0.1)
