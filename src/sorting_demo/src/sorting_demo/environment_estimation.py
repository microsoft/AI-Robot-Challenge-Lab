...
class EnvironmentEstimation:
    ...
    def compute_block_pose_estimation_from_arm_camera(self, CUBE_SIZE=150):
        ...
        if self.hand_camera_helper is not None:
            ...
            closest_block = self.identify_block_from_aproximated_point(camwtrans)
    ...
    def calculate_best_approximation_quaternion_for_cube(self, cube_orientation):
        ...
        # Get quaternion
        return tf.transformations.quaternion_from_matrix(rotation_matrix)
            ...
            if self.head_camera_helper is not None:
                ...
                for block in self.blocks:
                    if -0.43 < block.gazebo_pose.position.y < 0.43:
                        ...
                        block.headview_proj_estimation = block.gazebo_pose.position
                        block.hue_estimation = color_table[block.color]
                        block.headview_pose_estimation = Pose(
                            position=Point(x=block.headview_proj_estimation.x, y=block.headview_proj_estimation.y, z=block.headview_proj_estimation.z),
                            orientation=Quaternion(x=0, y=0, z=0, w=1))
